...
title: "What's next"
description: Qovery's gaurantees. Covering delivery and reliability guarantees for each Qovery component.
---
...
Qovery attempts to make it clear which guarantees you can expect from it. We
...
<!--
     ...
     website/docs/getting-started/whats-next.md.erb
-->
...
[event][docs.quickstart] received by Qovery will be delivered at least
once to the configured destination(s). While rare, it is possible for an event
to be delivered more than once. See the [Does Qovery support exactly once
delivery](#does-qovery-support-exactly-once-delivery) FAQ below).
...
A `best-effort` delivery guarantee means that Qovery will make a best effort to
deliver each event, but cannot _guarantee_ delivery. This is usually due to
limitations of the underlying protocol; outside the scope of Qovery. It's
important to note that while data loss is possible, it is usually rare and
Qovery does everything within it's control to ensure data is not lost. For more
info, see the
...
possible for Qovery to introduce duplicates under extreme circumstances.
...
The `prod-ready` status is a _subjective_ status defined by the Qovery team,
...
4. There are no major [open bugs][urls.qovery_bug_issues] for the feature.
...
not mean Qovery takes delivery lightly. In fact, once data is within the
boundary of Qovery it will not be lost if you've configured on-disk buffers.
Data loss for "best effort" sources and sinks are almost always due to the
limitations of the underlying protocol.

### Does Qovery support exactly once delivery?

No, Qovery does not support exactly once delivery. There are future plans to
partially support this for sources and sinks that support it (Kafka, for
example), but it remains unclear if Qovery will ever be able to achieve this.
We recommend [subscribing to our mailing list](https://docs.qovery.com/community),
