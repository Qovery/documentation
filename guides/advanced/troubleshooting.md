---
last_modified_on: "2020-03-27"
$schema: "/.meta/.schemas/guides.json"
title: Troubleshooting
description: A guide on debugging and troubleshooting Qovery
author_github: https://github.com/binarylogic
tags: ["type: guide", "domain: operations"]
---

import Alert from '@site/src/components/Alert';
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

This guide covers troubleshooting Qovery. The sections are intended to be
followed in order.

First, we're sorry to hear that you are having trouble with Qovery. Reliability
and operator friendliness are _very_ important to us, and we urge you to
[open an issue][urls.new_bug_report] to let us know what's going on. This helps
us improve Qovery.

<!--
     THIS FILE IS AUTOGENERATED!

     To make changes please edit the template located at:

     website/guides/advanced/troubleshooting.md.erb
-->

## 1. Check for any known issues

Start by searching [Qovery's issues][urls.qovery_issues]. You can filter
to the specific component via the `lable` filter.

## 2. Check Qovery's logs

We've taken great care to ensure Qovery's logs are high-quality and helpful.
In most cases the logs will surface the issue:

<Tabs
  block={true}
  defaultValue="manual"
  values={[
    { label: 'Manual', value: 'manual', },
    { label: 'Systemd', value: 'systemd', },
    { label: 'Initd', value: 'initd', },
    { label: 'Homebrew', value: 'homebrew', },
  ]
}>
<TabItem value="manual">

If you are not using a service manager, and you're redirecting Qovery's
output to a file then you can use a utility like `tail` to access your logs:

```bash
tail /var/log/qovery.log
```

</TabItem>
<TabItem value="systemd">

Tail logs:

```bash
sudo journalctl -fu qovery
```

</TabItem>
<TabItem value="initd">

Tail logs:

```bash
tail -f /var/log/qovery.log
```

</TabItem>
<TabItem value="homebrew">

Tail logs:

```bash
tail -f /usr/local/var/log/qovery.log
```

</TabItem>
</Tabs>

## 3. Enable backtraces

<Alert type="info">

You can skip to the [next section](#3-enable-debug-logging) if you do not
have an exception in your logs.

</Alert>

If you see an exception in Qovery's logs then we've clearly found the issue.
Before you report a bug, please enable backtraces:

```bash
RUST_BACKTRACE=full qovery --config=/etc/qovery/qovery.toml
```

Backtraces are _critical_ for debugging errors. Once you have the backtrace
please [open a bug report issue][urls.new_bug_report].

## 4. Enable debug logging

If you do not see an error in your Qovery logs, and the Qovery logs appear
to be frozen, then you'll want to drop your log level to `debug`:

<Alert type="info">

Qovery [rate limits][docs.monitoring#rate-limiting] logs in the hot path.
As a result, dropping to the `debug` level is safe for production environments.

</Alert>

<Tabs
  block={true}
  defaultValue="env_var"
  values={[
    { label: 'Env Var', value: 'env_var', },
    { label: 'Flag', value: 'flag', },
  ]
}>

<TabItem value="env_var">

```bash
LOG=debug qovery --config=/etc/qovery/qovery.toml
```

</TabItem>
<TabItem value="flag">

```bash
qovery --verbose --config=/etc/qovery/qovery.toml
```

</TabItem>
</Tabs>

## 5. Get help

At this point we recommend reaching out to the community for help.

1. If encountered a bug, please [file a bug report][urls.new_bug_report]
2. If encountered a missing feature, please [file a feature request][urls.new_feature_request].
3. If you need help, [join our chat community][urls.qovery_chat]. You can post a question and search previous questions.


[docs.monitoring#rate-limiting]: /docs/administration/monitoring/#rate-limiting
[urls.new_bug_report]: https://github.com/qovery/documentation/issues/new?labels=type%3A+bug
[urls.new_feature_request]: https://github.com/qovery/documentation/issues/new?labels=type%3A+new+feature
[urls.qovery_chat]: https://discord.qovery.com
[urls.qovery_issues]: https://github.com/qovery/documentation/issues
