...
description: "Qovery's field path notation allows you to reference field values with a simple string syntax."
---

Throughout Qovery's configuration you'll notice that certain options take field
paths as values, such as the
[`rename_fields` transform][docs.transforms.rename_fields]. In order to
referenxe nested, or array, values you can use Qovery's field path notation.
