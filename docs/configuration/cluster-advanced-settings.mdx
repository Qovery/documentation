...
On clusters running Kubernetes 1.28 or later, AWS automatically extends the use of this same key to encrypt other Kubernetes API data as well (e.g. ConfigMaps), not just Secrets — this is AWS's default behavior for 1.28+ clusters and applies regardless of Qovery's configuration. It does not cover node/EBS volume data, which is encrypted separately.
