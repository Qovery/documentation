...
<!--
     ...
     website/guides/advanced/deploy-backend-and-frontend-apps.md.erb
-->
...
You can send requests from one application to another using values injected automatically by Qovery into your [environment variables][docs.using-qovery.configuration.environment-variable] (called Built_in). 
For this example, let's assume that the internal host of the backend application is QOVERY_APPLICATION_ZBACKENDID_HOST_INTERNAL. We will define an alias for this built_in environment variable called `BACKED_HOST`, in this way we won't have to change our code if we deploy the same application on another environment (it will be "back-end_id indipendent"),

Examplary Node.js code snippet:

```javascript
const axios = require('axios');
const secondAppAddress = "http://" + process.env.BACKED_HOST
