...
function NavLink({href, hideIcon, label, onClick, position, right, to}) {
    ...
    const linkClassName = typeof window !== 'undefined' && (window.location.pathname.includes("/tutorial") || window.location.pathname.includes("/engineering")) && toUrl === "/guides/" ? "navbar__link" : "navbar__link--active"
    ...
    return (
        <Link
            ...
            {attributes.badge &&
            <span className={classnames('badge', `badge--${attributes.badgeStyle || 'secondary'}`)}>{attributes.badge}</span>}
        </Link>
    );
}
