...
The RDE Portal is a web application deployed on your own Qovery infrastructure. It consists of three components: a **React frontend**, an **Express BFF** (Backend-for-Frontend), and a **PostgreSQL database**. The portal communicates with Qovery's API and WebSocket services to manage workspaces on your behalf.
...
<CardGroup cols={2}>
  ...
  <Card title="Admin Client" icon="shield-halved">
    Uses an **organization admin API token** to execute privileged operations. This token enables the portal to perform actions like cloning environments across projects, deploying workspaces, and managing lifecycle — operations that individual users may not have direct permissions for.
  </Card>
</CardGroup>
...
The RDE Portal is deployed as **three services** on a Qovery environment within your infrastructure:

```mermaid
flowchart TB
    subgraph QoveryEnv["Qovery Environment"]
        FE["Frontend (nginx)"]
        BFF["BFF (Node.js 22)"]
        DB["PostgreSQL 16"]
    end
    ...
    FE -->|"API calls"| BFF
    BFF -->|"SQL"| DB
    BFF -->|"REST & WebSocket"| ExtAPI["Qovery Services"]

    style FE fill:#642DFF,color:#fff
    style BFF fill:#642DFF,color:#fff
    style DB fill:#642DFF,color:#fff
    style QoveryEnv fill:#f5f3ff,color:#1a1a2e
    style Users fill:#0d9488,color:#fff
    style ExtAPI fill:#1a1a2e,color:#fff
```

| Service | Type | Details |
|---------|------|---------|
| **Frontend** | Container (nginx) | Serves the Vite production build as static files |
| **BFF** | Container (Node.js 22) | Runs the Express server with all business logic |
| **PostgreSQL** | Database | Managed or container PostgreSQL 16 instance |

An optional **cleanup cron job** can be deployed alongside these services to automatically stop or delete stale workspaces based on idle time.
...
<CardGroup cols={2}>
  ...
  <Card title="Admin Setup" icon="gear" href="/rde/getting-started/admin-setup">
    Deploy and configure the portal on your infrastructure.
  </Card>
