...
description: "A full list of Qovery's supported environment variables and how to use them."
...
You can control Qovery's behavior through select environment variables:

```bash
ENV_VAR1=val ENV_VAR2=val qovery --config=/etc/qovery/qovery.toml
```
...
Sets Qovery's log level. See the [log section in the monitoring
...
Qovery supports custom environment variables via the `${...}` syntax:
...
[reloading][docs.process-management#reloading] Qovery.
