...
function Component({delivery_guarantee, description, event_types, function_category, logo_path, name, pathTemplate, status, title, type}) {
    ...
    return (
        <Link to={path} className="qovery-component" title={description}>
            <div className="qovery-component--header">
                <div className="qovery-component--name">
                    {title}
                </div>
            </div>
            ...
            <>
                {sourceComponents.length > 0 ?
                    <>
                        ...
                        <div className="qovery-components--grid">
...
function QoveryComponents(props) {
    //
    // Base Variables
    //
    ...
    const eventTypes = onlyEventTypes.size > 0 ?
        onlyEventTypes :
        new Set(
            _(components).map(component => component.event_types).flatten().uniq().compact().sort().value()
        );

    const operatingSystems = new Set(
        _(components).map(component => component.operating_systems).flatten().uniq().compact().sort().value());

    const serviceProviders = new Set(
        _(components).map(component => component.service_providers).flatten().uniq().compact().sort().value());

    const sourceFunctionCategories = new Set(
        _(components).filter(component => component.type == "source").map(component => component.function_category).uniq().compact().sort().value());

    const transformFunctionCategories = new Set(
        _(components).filter(component => component.type == "transform").map(component => component.function_category).uniq().compact().sort().value());


    const sinkFunctionCategories = new Set(
        _(components).filter(component => component.type == "sink").map(component => component.function_category).uniq().compact().sort().value());
    ...
    return (
        <div className={classnames('qovery-components', {'qovery-components--cols': filterColumn})}>
            <div className="filters">
                ...
                {operatingSystems.size > 0 && (
                    <div className="filter">
                        <div className="filter--label">
                            <Link to="/docs/setup/installation/operating-systems/" title="Learn more about Qovery's operating systems">
                                Operating Systems
                            </Link>
                        </div>
                        <div className="filter--choices">
...
export default QoveryComponents;
