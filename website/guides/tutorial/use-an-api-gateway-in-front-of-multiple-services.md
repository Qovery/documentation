...
<!--
     ...
     website/guides/tutorial/use-an-api-gateway-in-front-of-multiple-services.md.erb
-->
...
* `XXX` -> ALIAS -> `BILLING_BACKEND` with scope `ENVIRONMENT`
* `YYY` -> ALIAS -> `MESSAGING_BACKEND` with scope `ENVIRONMENT`
* `ZZZ` -> ALIAS -> `CORE_BACKEND` with scope `ENVIRONMENT`

One more environment variable must be added for the API Gateway.

* If your environment runs on AWS: `KUBERNETES_DNS_SERVICE_IP` with value `127.0.0.1` and scope `ENVIRONMENT`
* If your environment runs on Digital Ocean: `KUBERNETES_DNS_SERVICE_IP` with value `127.0.0.1` and scope `ENVIRONMENT`
* If your environment runs on Scaleway: `KUBERNETES_DNS_SERVICE_IP` with value `127.0.0.1` and scope `ENVIRONMENT`
