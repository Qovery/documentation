...
On you first sign-in into the [Qovery Console][urls.qovery_console], you need to login via your Git provider account. This allows to manage the access permission within your Qovery organization but it also allows Qovery to access the repositories linked to your Git account.
...
Git tokens are configured within the Git provider interface and then added into your Qovery organization to manage the access permission to your repositories.
...
<!--
     ...
     website/docs/using-qovery/configuration/organization/git-repository-access.md.erb
-->
...
Github offers two types of token: `Personal access tokens (classic)` and `Fine-grained personal access tokens`. You can read more about them and how to create them [here](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens).
...
Gitlab provides multiple type of tokens but Qovery supports two: `Project Tokens` and `Group Tokens`. You can find how to create them within these sections:
...
### Bitbucket
Bitbucket offers two types of token: `Repository access tokens` and `Workspaes access tokens` (only with Bitbucket Cloud Premium plan). You can read more about them and how to create them here:
...
2. Modify the existing token on the Qovery console by updating its value with the token created at step 1.

## Managing the tokens on Qovery

Tokens are centrally manaegd within your organization settings under the `Git repository access` section:
...
Once the token is created, you can use it to configure your Qovery services.
...
Note: If you want to modify the git token configured in Qovery, you can directly edit the token value. It will avoid you to manually update every application using the old token.
