...
<Note>
**The Self-Service Portal is now available in preview.** Full documentation is available in the dedicated [Remote Dev Environments Portal](/rde/overview) section.
</Note>
...
The portal is accessible at [rde.qovery.com](https://rde.qovery.com) and requires a **Business** or **Enterprise** plan with the RDE Portal add-on. Non-technical builders can:

- **Log in via SSO** through your Qovery organization
- **Browse available blueprints** configured by platform engineers
- **Create a workspace with one click** — pick a blueprint, click "Launch", and start building
- **Develop with built-in AI tools** — Claude Code, OpenCode chat, and a standard terminal
- **Preview running applications** in the browser with viewport switching
- **Publish to production** with admin approval workflows

Your source code, AI interactions, and terminal sessions stay on your Kubernetes cluster — workspace containers run entirely on your infrastructure while the portal handles orchestration.

<CardGroup cols={2}>
  <Card title="Portal Documentation" icon="book" href="/rde/overview">
    Full guide covering admin setup, blueprint configuration, workspace usage, and more.
  </Card>
