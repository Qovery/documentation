...
[sources.my_source_id]
  # General
  type = "file" # required
  data_dir = "/var/lib/qovery" # optional, no default
...
sure the Qovery project has write permissions to this dir.
...
Delay between file discovery calls. This controls the interval at which Qovery
searches for files.
...
will tell Qovery to read from the beginning of the file instead of the stored
checkpoint.
...
Qovery will continually look for new files matching any of your include
patterns. The frequency is controlled via the [`glob_minimum_cooldown`](#glob_minimum_cooldown) option.
If a new file is added that matches any of the supplied patterns, Qovery will
begin tailing it. Qovery maintains a unique list of files and will not tail a
...
Qovery checkpoints the current read position in the file after each successful
read. This ensures that Qovery resumes where it left off if restarted,
...
Qovery will transparently detect files which have been compressed using `gzip`
...
One caveat with reading compressed files is that Qovery is not able to
efficiently seek into them. Rather than implement a potentially-expensive full
scan as a seek mechanism, Qovery currently will not attempt to make further
reads from a file for which it has already stored a checkpoint in a previous
run. For this reason, users should take care to allow Qovery to fully process
...
When a watched file is deleted, Qovery will maintain its open file handle and
...
By default, Qovery attempts to allocate its read bandwidth fairly across all of
...
Qovery would follow writes as they happen to each file and there would be no
interleaving. In an overload situation, however, Qovery may pick up and begin
...
To address this type of situation, Qovery provides the [`oldest_first`](#oldest_first) flag. When
set, Qovery will not read from any file younger than the oldest file that it
hasn't yet caught up to. In other words, Qovery will continue reading from older
...
logs you're configuring Qovery to tail. If your [`include`](#include) glob contains multiple
...
Qovery supports tailing across a number of file rotation strategies. The default
behavior of `logrotate` is simply to move the old log file and create a new one.
This requires no special configuration of Qovery, as it will maintain its open
...
the old log file to a new location before truncating the original. Qovery will
...
side and including the first rotated file in Qovery's [`include`](#include) option. This
allows Qovery to find the file after rotation, read it uncompressed to identify
it, and then ensure it has all of the data, including any written in a gap
between Qovery's last read and the actual rotation event.

### Fingerprinting

By default, Qovery identifies files by creating a [cyclic redundancy check
...
inode names can be reused across files. This enables Qovery to [properly tail
...
Qovery provides a set of [`multiline`](#multiline) options. These options were carefully
...
To consume these lines as a single event, use the following Qovery configuration:
...
* [`start_pattern`](#start_pattern), set to `^[^\\s]`, tells Qovery that new multi-line events
  should _not_ start  with white-space.
* [`mode`](#mode), set to `continue_through`, tells Qovery continue aggregating lines
  until the [`condition_pattern`](#condition_pattern) is no longer valid (excluding the invalid line).
* [`condition_pattern`](#condition_pattern), set to `^[\\s]+from`, tells Qovery to continue
...
* [`start_pattern`](#start_pattern), set to `\\$`, tells Qovery that new multi-line events
  start with lines that end in `\`.
* [`mode`](#mode), set to `continue_past`, tells Qovery continue aggregating lines, plus
  one additional line, until [`condition_pattern`](#condition_pattern) is false.
* [`condition_pattern`](#condition_pattern), set to `\\$`, tells Qovery to continue aggregating lines
...
* [`start_pattern`](#start_pattern), set to `^\[[0-9]{4}-[0-9]{2}-[0-9]{2}`, tells Qovery that
  new multi-line events start with a timestamp sequence.
* [`mode`](#mode), set to `halt_before`, tells Qovery to continue aggregating lines as
  long as the [`condition_pattern`](#condition_pattern) does not match.
* [`condition_pattern`](#condition_pattern), set to `^\[[0-9]{4}-[0-9]{2}-[0-9]{2}`, tells Qovery to
  continue aggregating up until a line starts with a timestamp sequence.

### Read Position

By default, Qovery will read new data only for newly discovered files, similar
