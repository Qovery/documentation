...
use topology::Config;
use qovery::{config_paths, event, generate, list, metrics, runtime, topology, trace, unit_test};
...
struct RootOpts {
    ...
    /// `/etc/qovery/qovery.toml` will be targeted.
    ...
    ///
    /// By default `qovery` will try and detect if `stdout` is a terminal, if it is
...
enum SubCommand {
    ...
    /// Generate a Qovery configuration containing a list of components.
...
struct Validate {
    ...
    /// Any number of Qovery config files to validate. If none are specified the
    /// default config path `/etc/qovery/qovery.toml` will be targeted.
    paths: Vec<PathBuf>,
}
...
fn main() {
    ...
    let version = qovery::get_version();
    ...
    if opts.watch_config {
        ...
        qovery::topology::config::watcher::config_watcher(
            config_paths.clone(),
            qovery::topology::config::watcher::CONFIG_WATCH_DELAY,
        )
...
fn read_configs(config_paths: &Vec<PathBuf>) -> Result<Config, Vec<String>> {
    let mut config = qovery::topology::Config::empty();
...
fn validate(opts: &Validate) -> exitcode::ExitCode {
    ...
    for config_path in paths {
        ...
        let config = qovery::topology::Config::load(file);
        let config = handle_config_errors(config);
