...
While Qovery's built-in [transforms][docs.reference.transforms] are fast, sometimes they are not expressive
...
Lua is an interpreted language embedded into Qovery. When a `lua` transform is created, it starts an instance
...
user-defined Lua functions which are called by Qovery at certain events.
...
global variables, but they are not Qovery-specific, and so out of scope of this guide.

## Conclusion

As you have witnessed by reading this guide, the power of Qovery comes from its flexibility. In addition to
providing a [rich set][docs.reference.transforms] of predefined transforms for building production-grade observability
pipelines, it makes it possible to write custom aggregations as Lua scripts. This allows each role of Qovery
