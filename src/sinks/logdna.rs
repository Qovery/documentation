    ...
    fn encode_event(&self, event: Event) -> Option<Self::Input> {
        ...
        if !map.contains_key("app") && !map.contains_key("file") {
            if let Some(default_app) = &self.default_app {
                ...
                map.insert("app".to_string(), json!("qovery"));
            }
        }
...
mod tests {
    ...
    fn encode_event() {
        ...
        event1.as_mut_log().insert("app", "notqovery");
        ...
        assert_eq!(event1_out.get("app").unwrap(), &json!("notqovery"));
        assert_eq!(event2_out.get("file").unwrap(), &json!("log.txt"));
        assert_eq!(event3_out.get("app").unwrap(), &json!("qovery"));
    }
    ...
    fn smoke() {
        ...
        assert!(query.contains("hostname=qovery"));
        ...
        for (i, line) in output.iter().enumerate() {
            ...
            assert_eq!(line.get("app").unwrap(), &json!("qovery"));
