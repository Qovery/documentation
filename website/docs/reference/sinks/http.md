...
A URI that Qovery can request in order to determine the service health.
...
<Field
  ...
  defaultValue={null}
  enumValues={null}
  examples={[{"Authorization":"${HTTP_TOKEN}"},{"X-Powered-By":"Qovery"}]}
  groups={[]}
  name={"`[header-key]`"}
  path={"headers"}
  relevantWhen={null}
...
set with the [`headers`](#headers) option. For convenience, Qovery also supports the
...
with the Qovery team by [opening an issue][urls.new_http_sink_issue].
