...
Show whether the CLI is currently authenticated, as whom, which organization is selected, and when the session expires:

```bash
qovery auth status [flags]
```

This command never prints the access token or any other secret value, regardless of flags. It is the safe way to check authentication from scripts, CI, and automation, instead of parsing the output of `qovery auth token`.
...
The exit code is `0` when authenticated and `1` otherwise, so you can branch on it directly:
