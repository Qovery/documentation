...
<AccordionGroup>
  ...
  <Accordion title="How does Qovery schedule managed cluster maintenance?">
    ...
    <Info>
    Maintenance operations are performed using rolling updates, so your applications continue running without interruption during the process.
    </Info>
    ...
    **Opting out of a maintenance window**
    ...
    If you need to prevent maintenance from running on a specific cluster (e.g. during a critical release), you can lock it via the API:

    ```bash
    curl -X POST "https://api.qovery.com/cluster/<your-cluster-id>/lock" \
      -H "Authorization: Token <your-api-token>" \
      -H "Content-Type: application/json" \
      -d '{"reason": "critical release in progress"}'
    ```

    To unlock the cluster once you're ready:

    ```bash
    curl -X DELETE "https://api.qovery.com/cluster/<your-cluster-id>/lock" \
      -H "Authorization: Token <your-api-token>"
    ```

    You can generate an API token from your [account settings](https://console.qovery.com/user/api-tokens).
