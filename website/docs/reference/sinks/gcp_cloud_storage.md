...
The Qovery `gcp_cloud_storage` sink
...
authenticate access to the Cloud Storage API. If this is unset, Qovery checks
...
If `true` (the default), Qovery will validate the TLS certificate of the remote
host.
...
If `true` (the default), Qovery will validate the configured remote host name
...
objects. In the context of Qovery, only object ACLs are relevant (Qovery
...
By default, Qovery will name your GCS objects in the following format:
...
with the Qovery team by [opening an issue][urls.new_gcp_cloud_storage_sink_issue].
...
object level. In the context of Qovery only the object level is relevant
(Qovery does not create or modify buckets). You can set the storage
...
Qovery supports adding [custom metadata][urls.gcs_custom_metadata] to
