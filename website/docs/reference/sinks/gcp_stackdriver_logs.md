...
The Qovery `gcp_stackdriver_logs` sink
...
[sinks.my_sink_id]
  ...
  resource.projectId = "qovery-123456" # example
...
authenticate access to the Stackdriver Logging API. If this is unset, Qovery
...
<Field
  ...
  defaultValue={null}
  enumValues={null}
  examples={[{"projectId":"qovery-123456"},{"zone":"Twilight"}]}
  groups={[]}
  name={"`[label]`"}
  path={"resource"}
  relevantWhen={null}
...
with the Qovery team by [opening an issue][urls.new_gcp_stackdriver_logs_sink_issue].
