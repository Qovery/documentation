// NOTE: Tests assume that Kubernetes is accessable and localy available image of qovery
...
# Everything related to qovery should be in this namespace
apiVersion: v1
kind: Namespace
metadata:
   name: $(TEST_NAMESPACE)
...
// TODO: use localy builded image of qovery
pub static VECTOR_YAML: &'static str = r#"
# Qovery agent runned on each Node where it collects logs from pods.
apiVersion: apps/v1
kind: DaemonSet
metadata:
  name: qovery-agent
...
metadata:
  ...
  namespace: $(TEST_NAMESPACE)
  labels:
    qovery.test/label: echo
  annotations:
    qovery.test/annotation: echo
spec:
    ...
    pub fn wait_for_running(&self, mut object: KubeDaemon) -> KubeDaemon {
        ...
        retry(move || {
            ...
            match object.status.clone().ok_or("Object status is missing")? {
                ...
                status => {
                    // Try fetching Qoverys logs for diagnostic purpose
                    info_qovery_logs(self, &object);
...
pub fn start_qovery<'a>(
    kube: &Kube,
    user_namespace: &str,
    container_name: impl Into<Option<&'a str>>,
    config: &str,
) -> KubeDaemon {
    let qovery = create_qovery(kube, user_namespace, container_name, None, config);

    // Wait for running state
    kube.wait_for_running(qovery.clone());

    qovery
}

pub fn logs(kube: &Kube, qovery: &KubeDaemon) -> Vec<Value> {
    let mut logs = Vec::new();
    for daemon_instance in kube.list(&qovery) {
...
pub fn info_qovery_logs(kube: &Kube, qovery: &KubeDaemon) {
    for daemon_instance in kube.list(&qovery) {
...
fn kube_one_log() {
    ...
    // Start qovery
    let qovery = start_qovery(&kube, user_namespace.as_str(), None, CONFIG_MAP_YAML);
...
fn kube_diff_container() {
    ...
    // Start qovery
    let qovery = start_qovery(&kube, user_namespace.as_str(), "echo1", CONFIG_MAP_YAML);
...
fn kube_diff_namespace() {
    ...
    // Start qovery
    let qovery = start_qovery(&kube, user_namespace1.as_str(), None, CONFIG_MAP_YAML);
