...
* The [`journalctl`](#journalctl) binary is required, this is the interface Qovery uses to retrieve JournalD logs. See the ["Communication Strategy" section][docs.sources.journald#communication-strategy] for more info.
* The Qovery user must be part of the `systemd-journal` group in order to execute the [`journalctl`](#journalctl) binary. See the ["User Permissions" section][docs.sources.journald#user-permissions] for more info.
...
global [`data_dir`](#data_dir) is used. Please make sure the Qovery project has write
...
The full path of the [`journalctl`](#journalctl) executable. If not set, Qovery will search
...
Qovery checkpoints the journal position after every batch read. The size of
the batch is controlled via the [`batch_size`](#batch_size) option. Checkpointing ensures that
Qovery resumes where it left off if restarted, preventing data from being read
...
To ensure the `journald` source works across all platforms, Qovery interacts
with the Systemd journal via the [`journalctl`](#journalctl) command. This is accomplished by
spawning a [subprocess][urls.rust_subprocess] that Qovery diligently interacts
...
loss. Qovery handles this alternate format by translating such messages
into UTF-8 in "lossy" mode, where characters that are not valid UTF-8
are replaced with the Unicode replacement character, `�`.

### User permissions

If you run Qovery from a non-root user, you need to add that user to the
`systemd-journal` group.

For example, if the user is named `qovery`, it can be done by running

```sh
usermod -aG systemd-journal qovery
```
