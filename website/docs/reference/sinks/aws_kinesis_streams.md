...
description: "The Qovery `aws_kinesis_streams` sink batches `log` events to Amazon Web Service's Kinesis Data Stream service via the `PutRecords` API endpoint."
event_types: ["log"]
function_category: "transmit"
issues_url: https://github.com/qovery/documentation/issues?q=is%3Aopen+is%3Aissue+label%3A%22sink%3A+aws_kinesis_streams%22
operating_systems: ["Linux","MacOS","Windows"]
sidebar_label: "aws_kinesis_streams|[\"log\"]"
source_url: https://github.com/qovery/documentation/tree/master/src/sinks/aws_kinesis_streams.rs
status: "prod-ready"
...
The Qovery `aws_kinesis_streams` sink
...
<Field
  common={true}
  defaultValue={"memory"}
  enumValues={{"memory":"Stores the sink's buffer in memory. This is more performant, but less durable. Data will be lost if Qovery is restarted forcefully.","disk":"Stores the sink's buffer on disk. This is less performant, but durable. Data will not be lost between restarts."}}
...
Qovery checks for AWS credentials in the following order:
...
Qovery can assume an AWS IAM role via the [`assume_role`](#assume_role) option. This is an
...
shown in the diagram above. You'll notice that Qovery treats these concepts
differently, instead of treating them as global concepts, Qovery treats them
...
If the health check fails an error will be logged and Qovery will proceed to
start.
...
```bash
qovery --config /etc/qovery/qovery.toml --require-healthy
```
...
By default, Qovery issues random 16 byte values for each
...
Qovery will slice the value and use the first 256 characters.

#### Non-string values

Qovery will coerce the value into a string.

#### Provisioning & capacity planning

This is generally outside the scope of Qovery but worth touching on. When you
...
Qovery offers a few levers to control the rate and volume of requests to the
downstream service. Start with the [`rate_limit_duration_secs`](#rate_limit_duration_secs) and
`rate_limit_num` options to ensure Qovery does not exceed the specified
...
Please note, Qovery's defaults are carefully chosen and it should be rare that
you need to adjust these. If you found a good reason to do so please share it
with the Qovery team by [opening an issue][urls.new_aws_kinesis_streams_sink_issue].

### Retry Policy

Qovery will retry failed requests (status == `429`, >= `500`, and != `501`).
...
[urls.new_aws_kinesis_streams_sink_issue]: https://github.com/qovery/documentation/issues/new?labels=sink%3A+aws_kinesis_streams
