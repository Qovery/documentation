...
description: Building your very first Qovery pipeline, the "Hello World" of Qovery tutorials.
series_position: 1
...
Qovery is a simple beast to tame, in this guide we'll send an
...
If you haven't already, install Qovery. Here's a script for the lazy:
...
Qovery runs with a [configuration file][docs.configuration] that tells it which
...
* The [`stdin` source][docs.sources.stdin] tells Qovery to receive data over `STDIN`.
* The [`console` source]docs.sinks.stdout] tells Qovery to simply print the data to `STDOUT`.
* The  `encoding.codec` tells Qovery to print the data as plain text (unencoded).

</CodeExplanation>

Every component within a Qovery config has an identifier chosen by you. This
...
```bash
echo 'Hello World!' | qovery --config ./qovery.toml
```

<CodeExplanation>

* The `echo` statement sends a single log to Qovery over `STDIN`
* The `qovery...` command starts Qovery with our previously created config file.
...
text came out unchanged because we didn't ask Qovery to change it, we can remedy
