...
If you're using Qovery to write objects across AWS accounts then you should
...
objects. In the context of Qovery, only object ACLs are relevant (Qovery does
...
By default, Qovery will name your S3 objects in the following format:
...
Qovery appends a [UUIDV4][urls.uuidv4] token to ensure there are no name
conflicts in the unlikely event 2 Qovery instances are writing data at the same
time.
...
You'll notice that Qovery's [template sytax][docs.reference.templating] is
...
with the Qovery team by [opening an issue][urls.new_aws_s3_sink_issue].
...
of Qovery only the object level is relevant (Qovery does not create or modify
...
level. In the context of Qovery only the object level is relevant (Qovery does
not create or modify buckets). You can set the storage class via the
`storage_class` option.

### Tags & metadata

Qovery currently only supports [AWS S3 object tags][urls.aws_s3_tags] and does
...
support [Qovery's template syntax][docs.reference.templating], enabling dynamic
