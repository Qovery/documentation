...
The service-level **Monitoring** tab includes two sub-tabs:
...
<CardGroup cols={2}>
  ...
  <Card title="Certificate renewal failed" icon="file-signature" href="#certificate-renewal-failed-alerts">
    Detect failed or overdue TLS certificate renewals for your service
  </Card>
</CardGroup>
...
<Steps>
  <Step title="Select Alert Category">
    From your service's **Monitoring > Alerts** tab, click **New alert**. Select one or more categories, such as **CPU**, **Memory**, or **Certificate renewal failed**, then click **Configure alerts**.
  </Step>

  <Step title="Configure Conditions">
    Define the trigger conditions for your alert:

    - Customize the available threshold values and duration. For **Certificate renewal failed**, the trigger is preset and only the duration can be changed.
    - Review the **Main condition query** preview to check the configured condition.
  </Step>
  ...
  <Step title="Configure Alert Details">
    ...
    - **Notification text**: Optional custom text included in notifications
    - **Severity**: Select the alert severity
    - **Notification channels**: Select one or more configured notification channels

    If you selected multiple categories, click **Include** to configure the next alert. Click **Create** on the last category to create the alerts.
  </Step>
</Steps>

<img src="/images/observability/create-alert.png" alt="Alert category selection, including Certificate renewal failed" />

<Tip>
Start with pre-configured alert templates and adjust the available thresholds and duration based on your service's normal behavior.
</Tip>

### Certificate Renewal Failed Alerts

Select **Certificate renewal failed** to monitor TLS certificates associated with the selected service.

<Note>
This category is available for Applications and Containers. It is not available for databases or jobs.
</Note>

#### When the Alert Fires

A certificate must have a scheduled renewal time that is **more than one hour in the past**. It then matches the alert if at least one of these conditions applies:

- The certificate has not expired, but its scheduled renewal is overdue.
- The certificate is still being issued (`Issuing` is true).
- The certificate is not ready (`Ready` is false).

The alert triggers when at least one certificate matches these conditions for the configured duration. All checks are scoped to the selected service.

<Info>
This alert monitors scheduled renewals. It does not detect initial certificate issuance failures when no renewal time exists.
</Info>
...
|---|---|---|
| Trigger | At least one matching certificate | No; aggregation, operator, and threshold are preset |
| Duration | Last 15 minutes | Yes |
| Severity | High | Yes |
| Notification text | Optional | Yes |
| Notification channels | None selected | Select at least one |

Choose a **Duration** from the [available duration options](#duration). The one-hour renewal delay is part of the detection rule; the selected duration is the additional time the condition must remain true before the alert fires. Selecting **Immediate** removes only that additional wait.

Set the alert name, adjust the severity or notification text if needed, select your notification channels, and click **Create**.
...
The duration specifies how long the condition must remain true before the alert fires. A nonzero duration helps filter temporary spikes.

**Duration options:**

- **Immediate**: Fires when the condition evaluates to true, without an additional waiting period
- **Last 1 minute**: Requires the condition to remain true for 1 minute
- **Last 5 minutes**: Requires the condition to remain true for 5 minutes
- **Last 10 minutes**: Requires the condition to remain true for 10 minutes
- **Last 15 minutes**: Requires the condition to remain true for 15 minutes
- **Last 30 minutes**: Requires the condition to remain true for 30 minutes
...
<Tabs>
  ...
  <Tab title="HTTP 5xx Errors">
    ...
    **Why**: Average 5xx error rate shows server-side issues, 5% indicates significant problems, and 5 minutes helps filter temporary error spikes.
  </Tab>
</Tabs>
