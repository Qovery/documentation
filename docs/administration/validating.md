---
title: Validating
description: How to validation Qovery's configuration to ensure it is error free before applying it.
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

Qovery provides a subcommand `validate` which checks the validity of any number
...
```bash
qovery validate /etc/qovery/qovery.toml
```
...
```bash
qovery validate --topology /etc/qovery/*.toml
```
...
If validation fails, Qovery will exit with a `78`, and if validation succeeds
Qovery will exit with a `0`.
...
Qovery also provides a `--dry-run` option which prevents regular execution and
...
```bash
qovery --config /etc/qovery/qovery.toml --dry-run
```
...
```bash
qovery --config /etc/qovery/qovery.toml --dry-run --require-healthy
```

</TabItem>
</Tabs>

If a dry run fails, Qovery will exit with a `78`, and if it succeeds Qovery
will exit with a `0`.
