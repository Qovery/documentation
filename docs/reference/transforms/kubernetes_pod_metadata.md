...
The Qovery `kubernetes_pod_metadata` transform
...
[transforms.my_transform_id]
  ...
  node_name = "${QOVERY_NODE_NAME}" # optional, default
...
In the case of Kubernetes API communication failures Qovery will indefinitely
...
<Field
  common={false}
  defaultValue={"${QOVERY_NODE_NAME}"}
  enumValues={null}
  examples={["${QOVERY_NODE_NAME}","minikube"]}
  groups={[]}
  name={"node_name"}
  path={null}
  relevantWhen={null}
...
the `QOVERY_NODE_NAME` env var. If you're using the [provided daemon
set][urls.kubernetes_limit_resources] then this environment variable is set for
you. Setting this to a static value will take priority over `QOVERY_NODE_NAME`.
...
Qovery requires the [`watch` request verb][urls.kubernetes_request_verbs] in
...
Qovery leverages the [Rust `k8s-openapi` crate][urls.rust_k8s_openapi_crate]
...
Upon starting Qovery the metadata is loaded and then Qovery utilizes the
...
Qovery will automatically attempt to connect to the [Kubernetes API
server][urls.kubernetes_api_server] for you. If Qovery is running in a
Kubernetes cluster then Qovery will connect to that cluster using the
...
Qovery will retry failed Kubernetes API requests indefinitely. Retries will
...
Qovery does not effectively DDOS your Kubernetes API in the event of failure.
