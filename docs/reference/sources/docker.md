...
[sources.my_source_id]
  ...
  include_labels = ["com.example.vendor=Timber Inc.", "com.example.name=Qovery"] # optional, no default
```
...
<Field
  ...
  defaultValue={null}
  enumValues={null}
  examples={[["com.example.vendor=Timber Inc.","com.example.name=Qovery"]]}
  groups={[]}
  name={"include_labels"}
  path={null}
  relevantWhen={null}
...
<Field
  ...
  defaultValue={null}
  enumValues={null}
  examples={["ubuntu:latest","busybox","qovery/documentation:latest-alpine"]}
  groups={[]}
  name={"image"}
  path={null}
  relevantWhen={null}
...
<Field
  ...
  defaultValue={null}
  enumValues={null}
  examples={[{"com.example.vendor":"Timber Inc."},{"com.example.name":"Qovery"},{"com.example.build-date":"2029-04-12T23:20:50.52Z"}]}
  groups={[]}
  name={"`[label-key]`"}
  path={null}
  relevantWhen={null}
...
Qovery will automatically attempt to connect to the docker daemon for you. If
the user that Qovery is running under can run `docker ps` then Qovery will be
able to connect. Qovery will also respect if `DOCKER_HOST` and
...
2. Configure a compatible [Docker logging driver][urls.docker_logging_drivers]
   with a matching [Qovery source][docs.sources]. (advanced)

The Qovery `docker` source implements option 1. This is the simplest option,
...
First, it's worth mentioning that Qovery strives to guide you towards the
...
that side-step the Docker logging problems. For supported platforms see Qovery's
...
In order for the Qovery `docker` source to work properly, you must configure
...
This is a requirement of the [Docker daemon][urls.docker_daemon], which Qovery
...
difficult to work with. Qovery's `docker` source solves this by default,
