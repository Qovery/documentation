...
- You understand the [basic Qovery concepts][docs.about.concepts] and understand [how to set up a pipeline][guides.getting-started.your-first-pipeline].
---

Qovery has many built-in [parsers][pages.parse_transforms] for structured logs formats. However, when you need to ship
...
Let us draft an initial version of the Qovery's configuration file:
...
At this point, running `qovery --config qovery.toml` results in the following output:
...
directory where `qovery.toml` is stored:
...
With this `source` the `csv` module is loaded when Qovery is started up (or if the `lua` transform is added later and the
...
Trying to run `qovery --config qovery.toml` with the same input file results in structured events being output:
