...
- **Variable**: classic key/value pairs where the `value` can be retrieved at build and run time by using its `name` (key).
- **Variable as File**: key/value/path triplets where the `value` will be stored as a file on the specified `path`. Your application can then retrieve the `path` of the file at run time by using the variable `name` (key). Only text files are supported.

For each environment variable, you can also define:
