...
function GuidePage(props) {
    ...
    const {author_github: authorGithub, id, last_modified_on: lastModifiedOn, series_position: seriesPosition, title, description} = frontMatter;
        ...
        <Layout title={title} description={description}>
            {showSourceSwitcher && <Modal
                ...
                <QoveryComponents
                    ...
                    exceptNames={[source && source.name, 'docker', 'qovery']}
            ...
            {showSinkSwitcher && <Modal
                ...
                <QoveryComponents
                    exceptFunctions={['test']}
                    exceptNames={[sink && sink.name, 'qovery']}
                ...
                <div className="container">
                    {(platform || source || sink) && (
                        <div className="component-icons">
                            ...
                            {source && !platform &&
                            <div className="icon panel link" title="Change your source" onClick={(event) => setShowSourceSwitcher(true)}>
                                {source.logo_path ?
                                    <SVG src={source.logo_path} alt={`${source.title} Logo`}/> :
                                    <i className="feather icon-server"></i>}
                            </div>}
                            {!source && !platform &&
                            <div className="icon panel link" title="Select a source" onClick={(event) => setShowSourceSwitcher(true)}>
                                <i className="feather icon-plus"></i>
                            </div>}
                            {sink &&
                            <div className="icon panel link" title="Change your destination" onClick={(event) => setShowSinkSwitcher(true)}>
                                {sink.logo_path ?
                                    <SVG src={sink.logo_path} alt={`${sink.title} Logo`}/> :
                                    <i className="feather icon-database"></i>}
                            </div>}
                            {!sink &&
                            <div className="icon panel link" title="Select a destination" onClick={(event) => setShowSinkSwitcher(true)}>
                                <i className="feather icon-plus"></i>
                            </div>}
                        </div>
                    )}
                    ...
                    <section className={classnames('table-of-contents', styles.tableOfContents)}>
                        <div className="section">
                            ...
                            <div className="text--secondary text--bold">
                                <i className="feather icon-clock"></i> Updated <time pubdate="pubdate" dateTime={lastModifiedOn}>{dateFormat(lastModified, "mmm dS, yyyy")}</time>
                            </div>
                        </div>
