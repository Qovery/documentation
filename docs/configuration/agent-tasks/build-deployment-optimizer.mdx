...
<Steps>
  ...
  <Step title="Add the Qovery MCP server">
    ...
    To let it **apply** changes to Qovery directly instead, add the MCP server manually with a Policy API token that has write permissions on that service's build/deploy configuration. See [Add MCP Servers](/configuration/agent-tasks/overview#creating-an-agent).
  </Step>
