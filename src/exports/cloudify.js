...
export default function cloudify() {
  ...
  var Qovery = function(_x, _y) {

    this.x = _x || 0;
    this.y = _y || 0;

    this.dist = function(qovery) {
      var dx = this.x - qovery.x;
      var dy = this.y - qovery.y;
      return Math.sqrt(dx * dx + dy * dy);
    };

    this.minus = function(qovery) {
      return new Qovery(
        this.x - qovery.x,
        this.y - qovery.y);
    };
  ...
  var Node = function(_home, _label) {
    ...
    this.pos = new Qovery(_home.x, _home.y); // Current position
