...
The Qovery `tag_cardinality_limit` transform
...
Qovery will drop metric tags that exceed a cardinality of 1. Note, this limit is
...
as possible. This is because Qovery's cardinality cache is held in memory and it
will be erased when Qovery is restarted. This will cause new tag values to pass
through until the cardinality limit is reached again. For normal usage this
should not be a common problem since Qovery processes are normally long-lived.
...
This transform's cache is held in memory, and therefore, restarting Qovery
