...
Every catalog entry is identified by a **tag** in the form `{provider}/{service}/{majorVersion}/{semanticVersion}` — for example `AWS/postgres/17/1.2.6`. The tag pins the exact version of the template you deploy.

When you create a blueprint service, Qovery:

1. Reads the blueprint's manifest to build the input form — **variables** you fill in, plus read-only **context variables** (such as the cluster region and name) that Qovery resolves automatically from the target environment's cluster.
2. Provisions the real resource in your cluster (running `terraform apply` or `helm install` under the hood) and creates a managed Terraform or Helm service linked to the blueprint.
3. Exposes the service's **outputs** — endpoints, ports, credentials — so other services in the same environment can consume them.
