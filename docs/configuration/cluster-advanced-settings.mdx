...
<Warning>
This setting is available only for Qovery-managed EKS clusters that use Karpenter and a Qovery-managed VPC with NAT Gateways. It is not supported for clusters without NAT Gateways or clusters using a user-provided network.
</Warning>
...
**Learn More:** See [Envoy Gateway HTTPTimeout API Reference](https://gateway.envoyproxy.io/v1.8/api/extension_types/#httptimeout).
...
<a id="envoy-client-validation-ca-certificates"></a>
...
On clusters running Kubernetes 1.28 or later, Amazon EKS automatically enables envelope encryption for all Kubernetes API data (e.g. ConfigMaps), even when this setting is unset, using an AWS-owned key by default. If this setting supplies a customer-managed KMS key for Secrets, EKS also uses it for all Kubernetes API data. This does not cover node/EBS volume data, which is encrypted separately.
<Warning>
