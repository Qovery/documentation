...
# ENV VARS
#
#   $FEATURES - a list of Qovery features to include when building, defaults to all
...
archive_dir_name="qovery-$TARGET"
...
if [ "$STRIP" != "false" ]; then
  strip $target_dir/release/qovery
fi
...
# Copy the qovery binary to /bin
mkdir -p $archive_dir/bin
cp -av $target_dir/release/qovery $archive_dir/bin
...
if [[ $TARGET == *linux* ]]; then
  ...
  cp -av distribution/systemd/qovery.service $archive_dir/etc/systemd
  mkdir -p $archive_dir/etc/init.d
  cp -av distribution/init.d/qovery $archive_dir/etc/init.d
fi
...
if [ "$ARCHIVE_TYPE" == "tar.gz" ]; then
  tar cvf - ./$archive_dir_name | gzip -9 > qovery-$TARGET.$ARCHIVE_TYPE
elif [ "$ARCHIVE_TYPE" == "zip" ] && [[ $TARGET == *windows* ]]; then
  powershell '$progressPreference = "silentlyContinue"; Compress-Archive -DestinationPath qovery-'$TARGET'.'$ARCHIVE_TYPE' -Path "./'$archive_dir_name'/*"'
else
...
mv -v $target_dir/qovery-$TARGET.$ARCHIVE_TYPE $artifacts_dir
echo "Moved $target_dir/qovery-$TARGET.$ARCHIVE_TYPE to $artifacts_dir"
