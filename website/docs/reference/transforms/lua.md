...
issues_url: https://github.com/qovery/documentation/issues?q=is%3Aopen+is%3Aissue+label%3A%22transform%3A+lua%22
sidebar_label: "lua|[\"log\",\"metric\"]"
source_url: https://github.com/qovery/documentation/tree/master/src/transforms/lua
status: "beta"
title: "Lua Transform"
---
...
[transforms.my_transform_id]
  ...
  inputs = ["my-source-id"] # required
  version = "2" # required
  search_dirs = ["/etc/qovery/lua"] # optional, no default
...
Transform API version. Specifying this version ensures that Qovery does not
...
A function which is called when Qovery is stopped. It can produce new events
using `emit` function.
...
Events are represented as [tables][urls.lua_table] in Lua. Qovery uses
...
The correspondence between Qovery's [data types][docs.about.data-model.log#types] and Lua data type is summarized
by the following table:

| Qovery Type | Lua Type | Comment |
| :----------- | :-------- | :------- |
...
Qovery provides a [`search_dirs`](#search_dirs) option that allows you to specify absolute
...
Qovery uses the [`rlua` Rust crate][urls.rlua] which currently embeds Lua 5.3.
