...
Qovery supports a template syntax for select configuration options. This allows
you to use event fields when configuring Qovery:
...
Notice that Qovery allows direct field references as well as `strftime`
...
Qovery's [field notation][docs.reference.field-path-notation] is simple. It uses
...
In addition to directly accessing fields, Qovery offers a shortcut for injecting
...
Array values can be accessed using Qovery's
...
If a field is missing, a blank string will be inserted in it's place. Qovery
will not error, drop the event, or log anything.

### Nested Fields

Nested fields can be accessed using Qovery's
[field notation syntax][docs.reference.field-path-notation]:
