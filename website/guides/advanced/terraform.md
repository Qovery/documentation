---
last_modified_on: "2023-06-05"
$schema: "/.meta/.schemas/guides.json"
title: Terraform
description: Learn how to use Terraform with Qovery
author_github: https://github.com/evoxmusic
tags: ["type: guide", "technology: terraform"]
---
import Steps from '@site/src/components/Steps';
import Assumptions from '@site/src/components/Assumptions';
import Alert from '@site/src/components/Alert';

<Alert type="warning">

Follow [this guide][docs.using-qovery.integration.terraform] to learn more about Terraform with Qovery.

</Alert>

<!--
     THIS FILE IS AUTOGENERATED!

     To make changes please edit the template located at:

     website/guides/advanced/terraform.md.erb
-->

## Resources

Here are some additional resources you can use to learn more about Terraform integration with Qovery:

| Title                                                                                                          | Description                                                                                                                                          | Author    |
|----------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|
| [Deploy RDS instance with Terraform][guides.tutorial.how-to-use-lifecycle-job-to-deploy-any-kind-of-resources] | [Learn how to deploy an AWS RDS instance with Terraform and Lifecycle Job][guides.tutorial.how-to-use-lifecycle-job-to-deploy-any-kind-of-resources] | Qovery    |
| [Forum](https://discuss.qovery.com/search?q=terraform)                                                         | ["Terraform" threads from Qovery community forum](https://discuss.qovery.com/search?q=terraform)                                                     | Community |

## Q&A

Do you have any questions? Feel free to ask on our [Community forum][urls.qovery_forum].


[docs.using-qovery.integration.terraform]: /docs/using-qovery/integration/terraform/
[guides.tutorial.how-to-use-lifecycle-job-to-deploy-any-kind-of-resources]: /guides/tutorial/how-to-use-lifecycle-job-to-deploy-any-kind-of-resources/
[urls.qovery_forum]: https://discuss.qovery.com/
