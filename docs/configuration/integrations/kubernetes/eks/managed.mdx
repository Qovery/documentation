...
<Steps>
  ...
  <Step title="Set Resources">
    ...
    * **Spot instances**: In order to reduce even more your costs, you can also enable the spot instances on your clusters. Spot instances cost up to 90% less compared to On-Demand prices. But keep in mind that spot instances can be terminated by the cloud provider at any time. Check this [documentation](https://aws.amazon.com/ec2/spot/) for more information. This enables spot on the default node pool; the stable node pool stays on on-demand instances. Each node pool can be tuned later in the cluster settings.
...
- **Spot instances**: Enable or disable spot instances, per node pool. Enabling it on the stable node pool is discouraged: it hosts single-instance workloads that don't tolerate interruptions.
