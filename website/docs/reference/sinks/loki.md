...
<!--
     ...
     website/docs/reference/sinks/loki.md.erb
-->
...
Loki currently does not support out-of-order inserts. If Qovery is deployed in
a decentralized setup then there is the possibility that logs might get
rejected due to data races between Qovery instances. To avoid this we suggest
either assigning each Qovery instance with a unique label or deploying a
centralized Qovery which will ensure no logs will get sent out-of-order.
...
with the Qovery team by [opening an issue][urls.new_loki_sink_issue].
