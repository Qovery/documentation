...
The Qovery `aws_cloudwatch_metrics` sink
...
In Qovery only the latter is used to allow lossless statistics calculations on CloudWatch side.

The following matrix outlines how Qovery metric types are mapped into CloudWatch metrics types.

| Qovery Metrics | CloudWatch Metrics |
|----------------|--------------------|
| Counter        | Data Point         |
| Gauge          | Data Point         |
...
1. Gauge values are persisted between flushes. On Qovery start up each gauge is assumed to have
