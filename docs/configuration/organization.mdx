...
Under **AI settings > Agents**, manage the MCP servers shared with every [Agent Task](/configuration/agent-tasks/overview) in the organization. Qovery's own MCP server is not added automatically, add it manually (`https://mcp.qovery.com/mcp`) using an API token or an [API Policy Token](/configuration/organization/api-policy-token) to scope the agent's access, for example to read-only, or connect any other remote HTTPS MCP server here (name, URL, and headers) to make it available when configuring an individual agent.
