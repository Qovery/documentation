...
description: The various topologies that Qovery can accomplish and when to use each.
...
* **More efficient.** More efficient for client nodes and downstream services.
  Qovery agents are doing less which means they're using less resources. In
  addition, the centralized Qovery service will buffer more data, achieve
  better compression, and send optimized requests downstream.
* **More reliable.** Qovery will protect downstream services from volume spikes,
...
* **Less durable.** Agent nodes are designed to get data off of the machine as
  ...
  central Qovery service could die and lose buffered data. If this type of
...
* **Most efficient.** Qovery agents are doing less, making them more efficient,
  and Qovery services do not have to worry about durability, which can be tuned
  towards performance.
* **Ability to restream.** Restream your data depending on your stream's
  retention period.
* **Cleaner separation of responsibilities.** Qovery is used solely as a routing
