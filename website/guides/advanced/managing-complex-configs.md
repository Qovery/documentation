...
description: Strategies for building and managing complex Qovery configs
...
free Qovery configs that are easy to maintain.
...
for you to adopt it _before_ it's cool. Qovery supports complementing your
...
In Qovery each component of a pipeline specifies which components it consumes
...
Luckily, the Qovery team are desperate for your approval and have worked hard to
...
```bash
qovery generate /json_parser,add_fields,remove_fields > example.toml
# Find out more with `qovery generate --help`
```
...
With Qovery you can split a config down into as many files as you like and run
them all as a larger topology:

```bash
# These three examples run the same two configs together:
$ qovery -c ./configs/foo.toml -c ./configs/bar.toml
$ qovery -c ./configs/*.toml
$ qovery -c ./configs/foo.toml ./configs/bar.toml
```
...
Sometimes it's useful to update Qovery configs on the fly. If you find yourself
tinkering with a config that Qovery is already running you can prompt it to
reload the changes you've made by sending it a `SIGHUP` signal.

If you're running Qovery in environments where it's not possible to issue
