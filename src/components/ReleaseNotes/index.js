                            ...
                            {releases.map((menuRelease, idx) =>
                                <li className="menu__list-item" key={idx}>
                                    <Link to={`/releases/${menuRelease.version}`} className={classnames('menu__link', styles.sidebarMenuLink, {'menu__link--active': release == menuRelease})}>
                                        <div>{idx == 0 && <small className="badge badge--secondary" style={{
                                            marginRight: "5px",
                                            position: "relative",
                                            top: "-2px"
                                        }}>latest</small>} v{menuRelease.version}</div>
        ...
        <article className={styles.content}>
            <header className={styles.header}>
                <div className={styles.componentsHeroOverlay}>
                    <h1>Qovery v{release.version} Release Notes</h1>
                    <div className="hero--subtitle">
                        ...
                        <div>
                            <small>
                                ...
                                &nbsp;&nbsp;
                                <a href={release.type_url} target="_blank" className={classnames('badge', `badge--${releaseTypeClass}`, 'badge--rounded')} title={`This is a ${release.type} release as defined by the semantic versioning spec`}><i className="feather icon-chevrons-up"></i> {release.type}
                                </a>
                                &nbsp;&nbsp;
                                <a href={release.compare_url} target="_blank" className="badge badge--primary badge--rounded" title={`View the diff since ${release.last_version}`}>+{release.insertions_count},
                                    -{release.deletions_count}</a>
                            </small>
                        </div>
                    </div>
                    <div className="badges">
            ...
            <section className="markdown">
                ...
                <p>
                    We're excited to release Qovery v{release.version}! Qovery follows <a href="https://semver.org" target="_blank">semantic
                    versioning</a>, and this is an <a href={release.type_url} target="_blank">{release.type}</a> release. This release
                    brings <ChangelogSentence release={release}/>. Checkout the <a href="#highlights">highlights</a> for notable features
                    and, as always, <Link to="/community/">let us know what you think</Link>!
                </p>
                ...
                {release.upgrade_guides.length > 0 && (
                    <>
                        <AnchoredH2 id="breaking-changes" className="text--danger"><i className="feather icon-alert-triangle"></i> Breaking
                            Changes</AnchoredH2>
            ...
            <div className="table-of-contents">
                <div className="section">
                    ...
                    <ul className="contents">
                        ...
                        {release.upgrade_guides.length > 0 && (
                            <li>
                                <a href="#breaking-changes" className="contents__link text--danger"><i className="feather icon-alert-triangle"></i> Breaking
                                    Changes</a>
                                <ul>
                                    {release.upgrade_guides.map((upgradeGuide, idx) =>
                                        <li key={idx}>
...
function ReleaseNotes({version}) {
    ...
    return (
        <Layout title={`v${version} Release Notes`} description={`Qovery v${version} release notes. Highlights, changes, and updates.`}>
            <main>
