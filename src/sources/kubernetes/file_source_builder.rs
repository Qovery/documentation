...
impl<'a> FileSourceBuilder<'a> {
    ...
    ///
    /// By default it's good to exclude "kube-system" namespace and "qovery*" container name.
    /// But that default can/must be turned off if the user has anything included. The reason being:
    /// a) if user hasn't included  "kube-system" or "qovery*", than they will be filtered out
    ///    with include, so exclude isn't necessary.
    /// b) if user has included "kube-system" or "qovery*", then that is a sign that user wants
    ...
    fn file_source_exclude(config: &KubernetesConfig) -> Vec<String> {
        ...
        if no_include {
            ...
            // exclude kuberenetes and qovery logs.
            ...
            // NOTE: for now exclude images with name qovery, it's a rough solution, but necessary for now
            exclude.push((LOG_DIRECTORY.to_owned() + r"*/qovery*").into());
        }
