...
description: How to parse log data in Qovery
series_position: 2
...
Qovery makes it easy to mutate events into a more (or less) structured format
...
and Qovery has a much better way.
...
test qovery.toml: test apache regex ... failed
...
`qovery test ./qovery.toml` again and we should get this:

```text
Running qovery.toml tests
test qovery.toml: test apache regex ... passed
```
...
While we're at, did you know you can control all of Qovery's field names via
the [global `log_schema` options][docs.reference.global-options#log_schema]?
Qovery does not lock you into any specific schema.
...
Now that you're a Qovery pro you'll have endless ragtag groups of misfits
trying to recruit you as their hacker, but it won't mean much if you can't
deploy Qovery. Onto the next guide!
