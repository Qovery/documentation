...
#[derive(Debug, Snafu)]
pub enum Error {
    #[snafu(display("The configured data_dir {:?} does not exist, please create it and make sure the qovery process can write to it", data_dir))]
    DataDirNotFound { data_dir: PathBuf },
    #[snafu(display("The configured data_dir {:?} is not writable by the qovery process, please ensure qovery can write to that directory", data_dir))]
