...
<Steps>
  ...
  <Step title="Add the Qovery MCP server">
    Add Qovery's MCP server (`https://mcp.qovery.com/mcp`), with the HTTP header `Authorization` set to `Token <your policy API token>`. A read-only [API Policy Token](/configuration/organization/api-policy-token) is enough, this agent only reads and reports, it never applies a fix directly.
  </Step>
