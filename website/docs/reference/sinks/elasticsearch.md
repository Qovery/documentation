...
[sinks.my_sink_id]
  ...
  index = "qovery-%F" # optional, default
```
...
[sinks.my_sink_id]
  ...
  headers.X-Powered-By = "Qovery" # example

  # Query
  query.X-Powered-By = "Qovery" # example
...
field][urls.elasticsearch_id_field]. By default, Qovery does not set the `_id`
...
<Field
  common={true}
  defaultValue={"qovery-%F"}
  enumValues={null}
  examples={["application-{{ application_id }}-%Y-%m-%d","qovery-%Y-%m-%d"]}
  groups={[]}
  name={"index"}
  path={null}
  relevantWhen={null}
...
<Field
  ...
  defaultValue={null}
  enumValues={null}
  examples={[{"Authorization":"${ELASTICSEARCH_TOKEN}"},{"X-Powered-By":"Qovery"}]}
  groups={[]}
  name={"`[header-name]`"}
  path={"headers"}
  relevantWhen={null}
...
Qovery [batches](#buffers--batches) data flushes it to Elasticsearch's
...
with the Qovery team by [opening an issue][urls.new_elasticsearch_sink_issue].
