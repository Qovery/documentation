...
description: The various strategies used to deploy Qovery across your infrastructure.
...
Qovery is designed to be the single and only tool needed to get data from A to
B. This means Qovery can be deployed as a [daemon](#daemon),
...
* You are not responsible for the host machine and do not have the ability to
  install Qovery directly on the host.
* You want to couple data collection with each service, shifting responsibility
  to the service owner.
* You find it simpler for each service to have it own specific Qovery
  instance and configuration file.

### When NOT to use

* You do not like the idea of having multiple Qovery instances on a
  single host.
...
* Qovery is a stream consumer, continually pulling data from services like
  Kafka, Kinesis, or SQS.
* You are receving data from other upstream Qovery instances via the
  [`qovery` sink][docs.sinks.qovery].
