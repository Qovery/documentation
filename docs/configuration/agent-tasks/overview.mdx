...
<Steps>
  ...
  <Step title="Add MCP Servers (Optional)">
    Connect any remote HTTPS MCP server with its own name, URL, and headers. MCP servers added here are available to every agent task in the organization and can also be managed from **Organization Settings > AI settings > Agents**.

    Qovery's own MCP server isn't added automatically. To add it, use an [API Policy Token](/configuration/organization/api-policy-token) for scoped, read-only access (a regular API token works too):
  ...
  <Step title="Add a Trigger and Output (Optional)">
    Trigger the agent automatically, and optionally send its result somewhere when it's done. See [Triggers and Outputs](#triggers-and-outputs) below.
  </Step>
</Steps>
...
Each ready-made template has a sensible default: the Incident Analyzer and Jira Coding Agent templates default to Clone Environment since they can be triggered frequently or concurrently, while Build & deployment optimizer, typically run on a schedule, defaults to In Place.
...
Add a trigger to run the agent automatically, and optionally an output to send its result somewhere when it's done. You can also trigger any agent task on demand: click **Trigger** on its overview page, or from the environment's services list.
