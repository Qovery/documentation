...
- Kubernetes: applications deployed by Qovery on Kubernetes uses the [image pull policy](https://kubernetes.io/docs/concepts/containers/images/#image-pull-policy) `ifNotPresent`. This means that if the image already exists on the Kubernetes node local disk, Kubernetes won't pull again the image. If the image tag doesn't change, then the new image version won't be pulled and your pods will still run the old application code.
