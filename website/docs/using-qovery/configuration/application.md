...
# Deploying from a Container Registry (only available on the [Qovery Console V3](https://new.console.qovery.com))
...
- Image name: the name of the image to be deploying with this application (example: postgres)
- Image tag: the tag of the image to be deploying with this application (example: 12)
...
We have decided to not support the tag 'latest', please use a specific tag.
...
You can also expose ports publicly - it makes them accessible over the public network. Today we support only HTTPS, a dedicated public domain will be assigned to your application during the deployment.
...
- Go back to one of the previous steps and change your applicaiton settings
...
- Select the git provider where your code is stored (it can be hosted on GitHub, GitLab or Bitbucket).
- Select branch that Qovery should use for deploying your application
