...
At this stage, your file structure should look like this:
...
```title="Invoke application endpoint"
...
qovery application env add MONGO_URI '$QOVERY_DATABASE_MY_MONGO_CONNECTION_URI_WITHOUT_CREDENTIALS'
```

After running the command from above, we should see a new env variable added with the value of `QOVERY_DATABASE_MY_MONGO_CONNECTION_URI_WITHOUT_CREDENTIALS`:
...
Above, you can see that the env was added. Run the same command with `-c` flag to see that its value is in fact the same as `QOVERY_DATABASE_MY_MONGO_CONNECTION_URI_WITHOUT_CREDENTIALS`.
...
## Final words - scopes of env variables
...
so they're accessible by all applications in a given environment. The last scope - `PROJECT` scope - makes a given variable accessible to everywhere (all apps, all environments).
