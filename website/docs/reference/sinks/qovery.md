...
The Qovery `qovery` sink
[streams](#streaming) [`log`][docs.data-model.log] and
[`metric`][docs.data-model.metric] events to another downstream [`qovery`
source][docs.sources.qovery].
...
<!--
     ...
     website/docs/reference/sinks/qovery.md.erb
-->
...
The downstream Qovery address to connect to. The address _must_ include a port.
...
The `qovery` sink buffers events as shown in
...
The `qovery` sink streams data on a real-time
