...
import {Guide, GuideContent, GuideItemsToMetadata, GuidePaginated, GuideTags, PluginOptions, TagsModule,} from './types';
import {ConfigureWebpackUtils, LoadContext, Plugin, PluginContentLoadedActions,} from '@docusaurus/types';
    ...
    return {
        ...
        async contentLoaded({
            ...
            if (guideCategories.length > 0) {
                ...
                await Promise.all(
                    guideCategories.filter(category => !guidePermalinks.includes(category.permalink)).map(async category => {
                        ...
                        addRoute({
                            ...
                            exact: true,
                            modules: {
                                items: guides.filter(guide => guide.metadata.categories.map(category => category.permalink).includes(category.permalink)).map(guide => {
