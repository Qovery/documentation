...
This page covers installing Qovery from source using the native toolchain for
the host.

Qovery can also be compiled to a static binary for Linux for `x86_64`, `ARM64`,
...
The following steps should be used to compile Qovery directly on Linux based systems.
...
3.  ### Download Qovery's Source
    ...
    ```bash
    mkdir -p qovery && \
      curl -sSfL --proto '=https' --tlsv1.2 https://api.github.com/repos/qovery/documentation/tarball/v0.8 | \
...
4.  ### Change into the `qovery` directory
...
5.  ### Compile Qovery
    ...
    When finished, the qovery binary will be placed in
    `target/<target>/release/qovery`. For example, if you are building Qovery
    on your Mac, your target triple is `x86_64-apple-darwin`, and the Qovery
    binary will be located at `target/x86_64-apple-darwin/release/qovery`.

6.  ### Start Qovery

    Finally, start qovery:

    ```bash
    target/<target>/release/qovery --config config/qovery.toml
    ```
...
The steps to compile Qovery on Windows are different from the ones for other
operating systems.
...
4. ### Get Qovery's source using `git`
    ...
    ```bat
    git clone https://github.com/qovery/documentation
    git checkout v0.8.2
    cd qovery
    ```
...
5.  ### Build Qovery in release mode
...
6.  ### Start Qovery
    ...
    After these steps a binary `qovery.exe` in `target\release` would be
    created. It can be started by running

    ```bat
    .\target\release\qovery --config config\qovery.toml
    ```
...
It is possible to build statically linked binaries of Qovery for Linux using
Docker.
...
1.  ### Download Qovery's Source
    ...
    ```bash
    mkdir -p qovery && \
      curl -sSfL --proto '=https' --tlsv1.2 https://api.github.com/repos/qovery/documentation/tarball/v0.8.X | \
      tar xzf - -C qovery --strip-components=1
    ```
...
2.  ### Build Qovery using Docker
    ...
    `target/<target triple>/release/qovery` like in the previous case.
...
```bash
cp -av distribution/init.d/qovery /etc/init.d
```
...
```bash
cp -av distribution/systemd/qovery.service /etc/systemd/system
```
...
In addition, it is possible to pick only a subset of Qovery's components for
