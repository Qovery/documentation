...
issues_url: https://github.com/qovery/documentation/issues?q=is%3Aopen+is%3Aissue+label%3A%22transform%3A+logfmt_parser%22
sidebar_label: "logfmt_parser|[\"log\"]"
source_url: https://github.com/qovery/documentation/tree/master/src/transforms/logfmt_parser.rs
status: "beta"
...
The Qovery `logfmt_parser` transform
accepts and outputs [`log`][docs.data-model.log] events allowing you to parse a
log field's value in the [logfmt][urls.logfmt] format.
...
<Field
  common={true}
  defaultValue={null}
  enumValues={{"bool":"Coerces `\"true\"`/`/\"false\"`, `\"1\"`/`\"0\"`, and `\"t\"`/`\"f\"` values into boolean.","float":"Coerce to a 64 bit float.","int":"Coerce to a 64 bit integer.","string":"Coerce to a string.","timestamp":"Coerces to a Qovery timestamp. [`strptime` specificiers][urls.strptime_specifiers] must be used to parse the string."}}
...
support [Qovery's field notiation syntax][docs.reference.field-path-notation],
...
You can learn more about Qovery's field notation in the
...
is no official specification for the format and Qovery makes a best effort to
...
As noted above, if you do not specify a specific `strftime` format, Qovery
