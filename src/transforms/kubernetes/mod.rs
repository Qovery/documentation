...
/// Node name `spec.nodeName` of Qovery pod passed down with Downward API.
...
mod integration_tests {
    ...
    use crate::sources::kubernetes::test::{
        echo, info_qovery_logs, logs, start_qovery, user_namespace, Kube,
    };
...
"#;
    ...
    /// Starts a qovery with metadata transform with `fields` and an echo.
    ...
    fn kube_labels() {
        metadata_test("kube-labels", vec!["labels"], |value| {
            value
                ...
                .and_then(|labels| labels.get("qovery.test/label"))
                .and_then(|value| value.as_str())
                == Some("echo")
        });
    }
    ...
    fn kube_annotations() {
        metadata_test("kube-annotations", vec!["annotations"], |value| {
            value
                ...
                .and_then(|labels| labels.get("qovery.test/annotation"))
                .and_then(|value| value.as_str())
                == Some("echo")
        });
    }
