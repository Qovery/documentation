...
<Check>Qovery account with **organization admin** access</Check>
...
<Check>The RDE Portal deployed on your Qovery infrastructure — see [Architecture](/rde/reference/architecture)</Check>
...
<Steps>
  <Step title="Log in to the Portal">
    Open your portal URL (e.g., `https://rde.yourcompany.com`) and log in with your Qovery credentials via SSO. You must have **organization admin** access to see the admin panel.
  </Step>

  <Step title="Open Admin Settings">
    Navigate to **Admin > Settings** from the sidebar. This is where you configure the portal's connection to Qovery and customize its appearance.
  </Step>

  <Step title="Enter Your Qovery API Token">
    Paste a Qovery API token with **organization admin** privileges. This token allows the portal to perform privileged operations — cloning environments, managing deployments, and reading project configurations — on behalf of your users.

    <Warning>
    The API token is encrypted at rest using AES-256-GCM. It is never exposed to end users. Use a dedicated API token with admin-level access specifically for the portal.
    </Warning>
  </Step>

  <Step title="Customize Portal Branding">
    Configure the following settings to match your organization:
